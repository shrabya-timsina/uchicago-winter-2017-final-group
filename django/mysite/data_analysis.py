...
def create_agg_vectors(database, agg):
    ...
    user_vectors.to_csv('user_agg_vectors.csv')
...
def prepare_testvector(username, columns):
    ...
    test_dict = profile_scraper(user_url)
...
def topk_profiles_agg(username, topk, agg):
    ...
    '''
    user_vectors = pd.read_csv('user_agg_vectors.csv', index_col=0)
...
def get_beer_details_df(beer_id_list):
    '''
    ...
    beer_id_list_string = '(' + ','.join([str(beer_id) for beer_id in beer_id_list]) + ')'
    beer_details_df = pd.read_sql('SELECT DISTINCT beer_id, name, abv, style, brewery from beer_general_info where beer_id IN' + beer_id_list_string,
                            connect)
    connect.close()
    return beer_details_df
...
def get_suggestions_from_topk(username, k):
    ...
    '''
    topk_df = topk_profiles_agg(username, k, 'style')
