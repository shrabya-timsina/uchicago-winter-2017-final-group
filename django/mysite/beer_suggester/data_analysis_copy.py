...
def get_beer_details_df(beer_id_list):
    ...
    beer_details_df = pd.DataFrame(columns=['beer_id', 'name', 'abv', 'style', 'brewery'])
    for beer_id in beer_id_list:
        info_df = pd.read_sql('SELECT DISTINCT beer_id, name, abv, style, brewery from beer_general_info where beer_id =' + str(beer_id),
                            connect)
        beer_details_df.append(info_df, verify_integrity=True)
    connect.close()
